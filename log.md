...
### Round 3 — 网络增强 + 奖励塑形强化 (加载 Round 2 的 50万步黄金检查点)
...
```python
# 网络结构
NET_ARCH = [512, 512]         # 从 [256, 256] 增大，增强地形泛化

# 学习率
LEARNING_RATE = 1e-4            # 从 3e-4 降低，精细优化已有策略

# 探索
ENT_COEF = 0.1                  # 从 "auto" 固定为 0.1，维持中等探索
...
**备注**:
- Round 3 从 Round 2 的 50万步黄金检查点 (+527.78) 继续，而非从头或从 Round 1 加载
- 核心思路：在已学会行走的基础上，通过更强的奖励塑形和更大的网络，从"能走"进化到"走得快、走得稳"
- 如果 10 万步后评估奖励没有提升或下降，考虑回退到 `LEARNING_RATE = 3e-4` 或 `NET_ARCH = [256, 256]`
...
- [ ] **Round 1**: 完成 1000 万步基线训练（已启用奖励塑形）
- [ ] **调参实验**: 如果 Round 1 仍卡住，尝试增大 `STALL_PENALTY` 到 2.0 或 3.0
- [ ] **Round 2**: 尝试降低学习率 (1e-4) 或增大网络 (512, 512)
- [ ] **Round 3**: 尝试调整 ent_coef 为固定值 0.2

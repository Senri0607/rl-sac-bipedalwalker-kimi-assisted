...
> 经过 **7 轮迭代实验**，从基线（-109）到突破 **+2056**，并实现了接近人类的行走步态。
...
本项目通过**多轮迭代实验**（7 轮）探索了奖励塑形（Reward Shaping）的演进：
- 从**基线无塑形**（Round 1）
- 到**温和塑形**（Round 2，+527）
- 到**激进增强**（Round 3/4，失败）
...
**效果**：
- 独脚蹭行（速度 0.1）→ stride_bonus = 0.005（极少）
- 正常走路（速度 0.5）→ stride_bonus = 0.025（正常）
- 大步快跑（速度 1.0）→ stride_bonus = 0.05（最多）
...
> **建议**: 困难版强烈建议开启奖励塑形。当前版本使用**速度挂钩**机制，既能获得高分，又能实现接近人类的行走步态。
...
| 指标 | 数值 | 步数 |
|------|------|------|
| **最高评估奖励** | **+2056.50** | 1,200,000 (Round 7) |
| 标准差 | 21.33 | 极稳定 |
| Episode 长度 | 1108.50 ± 75.50 | 稳定 |
| 步态 | 接近人类（交替步态，自然抬腿） | 速度挂钩奖励 |
| 学习率 | 3e-4 | 未改变 |
| 网络结构 | [256, 256] | 未改变 |
